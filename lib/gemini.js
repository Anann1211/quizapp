...
export async function generateQuestions({ text, imageBase64, mimeType, count, type, previousQuestions, topic }) {
  ...
  model: 'gemini-2.5-flash-lite', // Dùng bản 1.5 để đọc được PDF trực tiếp
  generationConfig: { 
  ...
  const topicContext = topic
    ? `\nTRỌNG TÂM: Câu hỏi PHẢI xoay quanh chủ đề "${topic}" trong tài liệu. Ưu tiên nội dung liên quan đến chủ đề này.`
    : ''
...
try {
  ...
  throw new Error("AI không thể tạo cấu trúc dữ liệu chuẩn. Hãy thử lại.")
}
}
